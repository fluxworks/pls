...
use crate::terminal::{
    CursorMode, RawRead, SignalSet, Size, Terminal, TerminalReader, TerminalWriter,
};
...
impl MemoryTerminal {
    ...
    fn lock_reader(&self) -> MutexGuard<'_, Reader> {
...
impl Writer {
    fn new(size: Size) -> Writer {
        assert!(
            size.lines != 0 && size.columns != 0,
            "zero-area terminal buffer: {:?}",
            size
        );
...
impl<'a> TerminalReader<MemoryTerminal> for MemoryReadGuard<'a> {
    ...
    fn prepare(&mut self, _block_signals: bool, _report_signals: SignalSet) -> io::Result<()> {
        Ok(())
    }
    ...
    fn restore(&mut self, _state: ()) -> io::Result<()> {
        Ok(())
    }
...
mod test {
    ...
    fn assert_lines(mem: &MemoryTerminal, tests: &[&str]) {
        ...
        while let Some(line) = lines.next() {
            ...
            assert!(
                line.iter().cloned().eq(test.chars()),
                "mem: {:?}; tests: {:?}",
                mem.lock_writer().memory,
                tests
            );
        }
    }
    ...
    #[test]
    fn test_memory_term() {
        let mem = MemoryTerminal::with_size(Size {
            lines: 3,
            columns: 4,
        });
