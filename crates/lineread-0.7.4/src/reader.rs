...
use crate::terminal::{RawRead, Signal, SignalSet, Size, Terminal, TerminalReader};
...
pub(crate) struct Read<Term: Terminal> {
    ...
    /// Byte length of the common prefix of completion display strings (for highlighting)
    pub completion_display_prefix: usize,
    /// Whether to highlight the distinguishing suffix of completion candidates
...
pub(crate) enum InputState {
    Inactive,
    NewSequence,
    ContinueSequence { expiry: Option<Instant> },
    Number,
    CharSearch { n: usize, backward: bool },
    TextSearch,
    CompleteIntro,
    CompleteMore(usize),
    QuotedInsert(usize),
...
impl<'a, Term: 'a + Terminal> Reader<'a, Term> {
    ...
    fn read_line_step_impl(&mut self, timeout: Option<Duration>) -> io::Result<Option<ReadResult>> {
        let do_read = if self.lock.is_input_available() {
            ...
            self.lock
                .term
                .wait_for_input(Some(Duration::from_secs(0)))?
        } else {
    ...
    /// Enables or disables highlighting of completion candidate suffixes.
    pub fn set_highlight_completions(&mut self, enable: bool) {
        self.lock.highlight_completions = enable;
    }
...
impl<'a, Term: 'a + Terminal> ReadLock<'a, Term> {
    ...
    fn is_input_available(&self) -> bool {
        !self.data.macro_buffer.is_empty()
...
impl<Term: Terminal> Read<Term> {
    pub fn new(term: &Term, application: Cow<'static, str>) -> Read<Term> {
        let mut r = Read {
            ...
            completion_start: 0,
            completion_prefix: 0,
            completion_display_prefix: 0,
            highlight_completions: false,
