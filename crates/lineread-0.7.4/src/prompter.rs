...
use crate::highlighting::RESET_STYLE;
...
use crate::util::{
    back_n_words, backward_char, backward_word, find_matching_paren, first_word, forward_char,
    forward_n_words, forward_word, get_open_paren, longest_common_prefix, repeat_char, word_end,
    word_start, RangeArgument,
};
use crate::variables::VariableIter;
use crate::writer::{
    display_str, Digit, Display, HistoryIter, PromptType, WriteLock, Writer, BLINK_DURATION,
};
...
impl<'a, 'b: 'a, Term: 'b + Terminal> Prompter<'a, 'b, Term> {
    ...
    pub(crate) fn handle_input(&mut self, ch: char) -> io::Result<Option<ReadResult>> {
        ...
        match self.read.state {
            ...
            InputState::CompleteIntro => match ch {
                'y' | 'Y' | ' ' => {
                    self.write.write_str("\n")?;
            ...
            InputState::CompleteMore(offset) => match ch {
                'y' | 'Y' | ' ' => {
    ...
    fn execute_command(&mut self, cmd: Command, n: i32, ch: char) -> io::Result<()> {
        ...
        match cmd {
            ...
            UnixWordRubout => {
                if n > 0 {
                    let pos =
                        backward_word(n as usize, &self.write.buffer, self.write.cursor, " \t\n");
                    let r = pos..self.write.cursor;
                    self.kill_range(r)?;
                } else if n < 0 {
                    let pos = forward_word(
                    ...
                    if let Some(first) = first_word(
                        ...
                        if first != start {
                            ...
                            if !self.explicit_arg() && start == self.write.buffer.len() {
                                ...
                                let dest_end =
                                    word_end(&self.write.buffer, dest_start, &self.read.word_break);
                                ...
                                let src_end =
                                    word_end(&self.write.buffer, src_start, &self.read.word_break);
    ...
    fn check_expire_sequence(&mut self, now: Instant) -> io::Result<()> {
        if let InputState::ContinueSequence {
            expiry: Some(expiry),
        } = self.read.state
        {
            if now >= expiry {
    ...
    fn build_completions(&mut self) {
        ...
        if start > end {
            panic!(
                "Completer::word_start returned invalid index; \
                start > end ({} > {})",
                start, end
            );
        }
    ...
    fn complete_word(&mut self) -> io::Result<()> {
        if let Some(completions) = self.read.completions.take() {
            ...
            if completions.len() == 1 {
                ...
                {
                    let pfx = longest_common_prefix(
                        completions.iter().map(|compl| &compl.completion[..]),
                    )
                    .unwrap_or_default();
    ...
    fn show_completions(&mut self, completions: &[Completion]) -> io::Result<()> {
        ...
        // Compute display prefix as the longest common prefix of all display strings
        let lcp = longest_common_prefix(completions.iter().map(|s| &s[..])).unwrap_or("");
        self.read.completion_display_prefix = lcp.len();

        let cols = format_columns(
            &completions,
            eff_width,
    ...
    fn start_page_completions(&mut self, n_completions: usize) -> io::Result<()> {
        ...
        self.write
            .redraw_prompt(PromptType::CompleteIntro(n_completions))
    }
    ...
    fn write_completion_name(&mut self, name: &str) -> io::Result<()> {
        let pfx = self.read.completion_display_prefix;
        if self.read.highlight_completions && pfx > 0 && pfx < name.len() {
            self.write.write_str(&name[..pfx])?;
            self.write.write_str("\x1b[0;32m")?;
            self.write.write_str(&name[pfx..])?;
            self.write.write_str(RESET_STYLE)?;
        } else {
    ...
    fn show_completion_line<S: AsRef<str>>(&mut self, line: Line<S>) -> io::Result<()> {
        ...
        for (width, name) in line {
            ...
            self.write_completion_name(name)?;
    ...
    fn prev_completion(&mut self, n: usize) -> io::Result<()> {
        ...
        let old = self.read.completion_index;
        let new = if n <= old { max - old - n } else { old - n };
        ...
        if old != new {
            ...
            if new == len {
                ...
                let s = self.read.completions.as_ref().unwrap()[new]
                    .completion(self.read.completion_append_character)
                    .into_owned();
    ...
    pub fn kill_range<R: RangeArgument<usize>>(&mut self, range: R) -> io::Result<()> {
        ...
        let end = range
            .end()
            .cloned()
            .unwrap_or_else(|| self.write.buffer.len());
        let len = end - start;
