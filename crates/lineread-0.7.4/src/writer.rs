...
use crate::util::{
    backward_char, backward_search_char, filter_visible, forward_char, forward_search_char,
    grapheme_width, RangeArgument,
};
...
impl<'a, Term: Terminal> WriteLock<'a, Term> {
    ...
    pub fn blink(&mut self, pos: usize) -> io::Result<()> {
        ...
        self.blink = Some(Blink { pos, expiry });
    ...
    pub fn draw_prompt_suffix(&mut self) -> io::Result<()> {
        match self.prompt_type {
            ...
            PromptType::Search => {
                ...
                let adjusted_styles: Vec<(Range<usize>, Style)> = entry_styles
                    .into_iter()
                ...
                let (target_line, target_col) =
                    self.line_col_with(target_cursor_pos_in_full_str, &full_str, 0);
    ...
    fn draw_text_impl(
        ...
        if current_style != &Style::Default {
            ...
            let last_applied_style = styles
                .iter()
                .rev()
                .find(|(range, _)| range.start < text_offset + text.len());
    ...
    pub fn set_cursor(&mut self, pos: usize) -> io::Result<()> {
        ...
        if !self.buffer.is_char_boundary(pos) {
            panic!(
                "invalid cursor position {} in buffer {:?}",
                pos, self.buffer
            );
        }
...
impl Write {
    ...
    pub fn new_history(&self) -> Skip<HistoryIter<'_>> {
        let first_new = self.history.len() - self.history_new_entries;
...
pub(crate) fn display_str<'a>(s: &'a str, style: Display) -> Cow<'a, str> {
    if s.chars()
        .all(|ch| display(ch, style) == DisplaySequence::Char(ch))
    {
        Borrowed(s)
    } else {
