...
pub fn filter_visible(s: &str) -> Cow<'_, str> {
...
pub fn longest_common_prefix<'a, I, S>(iter: I) -> Option<&'a str>
where
...
pub fn backward_char(n: usize, s: &str, cur: usize) -> usize {
    let mut graphemes: Vec<(usize, &str)> = s[..cur].grapheme_indices(true).collect();
...
#[cfg(test)]
mod test {
    use super::{backward_char, forward_char, grapheme_width, longest_common_prefix, match_name};

    #[test]
    fn test_longest_common_prefix() {
        let empty: &[&str] = &[];

        assert_eq!(longest_common_prefix(empty.iter()), None);
        assert_eq!(longest_common_prefix(["foo", "bar"].iter()), None);
        assert_eq!(longest_common_prefix(["foo"].iter()), Some("foo"));
        assert_eq!(longest_common_prefix(["foo", "foobar"].iter()), Some("foo"));
        assert_eq!(longest_common_prefix(["foobar", "foo"].iter()), Some("foo"));
        assert_eq!(
            longest_common_prefix(["alpha", "alpaca", "alto"].iter()),
            Some("al")
        );

        assert_eq!(longest_common_prefix(["äöüx", "äöüy"].iter()), Some("äöü"));
    }
