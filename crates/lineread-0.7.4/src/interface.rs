...
use std::io::{self, BufRead, BufReader, BufWriter, Read as _, Seek, SeekFrom, Write as _};
use std::path::Path;
...
use crate::reader::{Read, ReadLock, ReadResult, Reader};
use crate::terminal::{DefaultTerminal, Signal, Terminal};
use crate::variables::Variable;
use crate::writer::{Write, WriteLock, Writer};
...
impl<Term: Terminal> Interface<Term> {
    ...
    pub fn lock_writer_append(&self) -> io::Result<Writer<'_, '_, Term>> {
    ...
    pub fn lock_writer_erase(&self) -> io::Result<Writer<'_, '_, Term>> {
        Writer::with_lock(self.lock_write(), true)
    }

    fn lock_read(&self) -> ReadLock<'_, Term> {
        ReadLock::new(
            self.term.lock_read(),
    ...
    pub(crate) fn lock_write(&self) -> WriteLock<'_, Term> {
        WriteLock::new(
    ...
    pub(crate) fn lock_write_data(&self) -> MutexGuard<'_, Write> {
...
impl<Term: Terminal> Interface<Term> {
    ...
    pub fn set_highlight_completions(&self, enable: bool) {
        self.lock_reader().set_highlight_completions(enable);
    }
